            ...
            for r in range(n_runs):
                ...
                for ete in np.argwhere(data['class'].to_numpy() != 0):
                    ...
                    if len(choose) > 0:
                        ...
                        pos = -1
                    print(choose_s)
                    dists_sod, pos_sod, _ = calcola_knn_score(x_train_sub[:, choose_s], y_train_sub,
                ...
                pickle.dump(f_choose, open(os.path.join(path, f.replace('info_', '').replace('.csv', f'_{r}_')+'choose.joblib'), 'wb'))
                pickle.dump(sod_choose, open(os.path.join(path, f.replace('info_', '').replace('.csv', f'_{r}_')+'sod_choose.joblib'), 'wb'))
                pickle.dump(new_points,
                                open(os.path.join(path, f.replace('info_', '').replace('.csv', f'_{r}_') + 'new_points.joblib'),
                                    'wb'))
    
                info_df = pd.read_csv(data_path + f.replace('synth_', 'info_synth_'))
                subspaces = pickle.load(
                    open(data_path + f.replace('synth_', 'info_synth_').replace('.csv', '.joblib'), 'rb'))
                prec_f = []
                sens_f = []
                ...
                pickle.dump(sens_f, open(os.path.join(path, f.replace('.csv', f'_{r}_test_sensibilities.joblib')), 'wb'))
                pickle.dump(prec_f, open(os.path.join(path, f.replace('.csv', f'_{r}_test_precisions.joblib')), 'wb'))
